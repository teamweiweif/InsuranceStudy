...
INPUT_ZIP = PROJECT_ROOT / "data" / "raw" / "feasibility_audit" / "sipp" / "2023" / "pu2023_csv.zip"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "prototype"
OUTPUT_PARQUET = OUTPUT_DIR / "sipp_2023_corrected_person_month_flags.parquet"
OUTPUT_SUMMARY = OUTPUT_DIR / "sipp_2023_correction_pilot_summary.json"
...
def load_2023_file() -> pd.DataFrame:
...
def build_corrected_flags(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    df["medicaid_spell_overrun"] = _overrun_mask(df["EMD_EMONTH"], df["MONTHCODE"])
    ...
    df["corr_EMD_EMONTH"] = df["EMD_EMONTH"].where(~df["medicaid_spell_overrun"], pd.NA).astype("Int16")
