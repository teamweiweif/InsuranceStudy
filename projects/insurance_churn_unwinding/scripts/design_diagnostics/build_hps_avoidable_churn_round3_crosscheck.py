...
ROUND3_SUMMARY_JSON = OUTPUT_DIR / "avoidable_churn_round3_robustness_summary.json"
STATE_MONTH_CSV = OUTPUT_DIR / "avoidable_churn_round3_state_month_cells.csv"

STATE_WEEK_CSV = OUTPUT_DIR / "hps_avoidable_churn_round3_crosscheck_state_week_cells.csv"
SUMMARY_CSV = OUTPUT_DIR / "hps_avoidable_churn_round3_crosscheck_summary.csv"
MD_PATH = OUTPUT_DIR / "hps_avoidable_churn_round3_crosscheck.md"
...
def load_candidate() -> tuple[str, str]:
    summary = json.loads(ROUND3_SUMMARY_JSON.read_text(encoding="utf-8"))
    top_core = summary["top_core_nonlead_exposures"]
    chosen = top_core[0]
    return chosen["exposure_variant"], chosen["alignment"]


def load_exposure(variant: str, alignment: str) -> pd.DataFrame:
    exp_col = f"{variant}_{alignment}"
    cols = ["reference_year", "tehc_st_fips", "MONTHCODE", exp_col]
...
def build_state_week_cells(exposure_df: pd.DataFrame) -> pd.DataFrame:
    state_week = pd.concat([load_hps_week(meta) for meta in WEEK_MAPS], ignore_index=True)
    return state_week.merge(
        exposure_df,
